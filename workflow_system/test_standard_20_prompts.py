"""
Standard tier validation test - 20 different prompts.
Tests duplicate fix across maximum business scenario diversity on Standard tier.
"""
import asyncio
import sys
...
async def test_prompt(prompt_num: int, test_case, engine):
    ...
    inquiry = EmailInquiry(
        message_id=f"standard-test-{prompt_num}",
    ...
    try:
        result, _ = await engine.process_inquiry(inquiry, "Standard")
...
async def main():
    print("=" * 80)
    print("STANDARD TIER - 20 PROMPT VALIDATION TEST")
    print("=" * 80)
    ...
    if passed == total:
        ...
        print("Duplicate bug fix is VALIDATED across 20 diverse scenarios on Standard tier.")
