...
class TestCompassHtmlSaving:
    ...
    async def test_list_html_results_includes_compass_files(self):
        """Test that listing results includes both workflow and compass HTML files."""
        import os
        from datetime import datetime

        # Create test_results directory with both types of files
        test_results_dir = "test_results"
        os.makedirs(test_results_dir, exist_ok=True)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        # Create workflow format file
        workflow_file = f"Standard_Test_Company_{timestamp}_abc12345.html"
        workflow_path = os.path.join(test_results_dir, workflow_file)
        with open(workflow_path, "w", encoding="utf-8") as f:
            f.write("<html><body>Workflow</body></html>")
        ...
        try:
            ...
            assert response.status_code == 200
            data = response.json()
            assert data["total"] == 2, f"Expected 2 files (workflow + compass), got {data['total']}"

            # Find compass result
            compass_results = [r for r in data["results"] if r["tier"] == "Compass"]
            assert len(compass_results) == 1, "Expected 1 compass result"
